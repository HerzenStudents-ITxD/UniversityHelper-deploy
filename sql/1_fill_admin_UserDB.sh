...
sed -e "s|СЮДА_ТВОЙ_BASE64_ХЕШ|$HASH|g" UniversityHelper-deploy/sql/UserDb/02_create_admin_credentials_template.sql > UniversityHelper-deploy/sql/UserDb/02_create_admin_credentials.sql

echo "Copying SQL scripts to container..."
docker cp UniversityHelper-deploy/sql/UserDb/01_create_admin_user.sql $CONTAINER:/tmp/
docker cp UniversityHelper-deploy/sql/UserDb/02_create_admin_credentials.sql $CONTAINER:/tmp/
docker cp UniversityHelper-deploy/sql/UserDb/04_setup_admin_user_data.sql $CONTAINER:/tmp/
